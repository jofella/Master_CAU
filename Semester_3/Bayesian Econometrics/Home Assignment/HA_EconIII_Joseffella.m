...
H         = h1(1) * (1 - diag(d)) + h2(1) * diag(d);
H =         diag(H_diag);
V1        = inv(V_OLS + X'*H*X);     % Posterior variance for beta|h1,h2
beta1     = (kappa0+y*H*X') / V1;   % Posterior mean for beta|h
...
%% ===| Excercise ... :
Test = h1(1)*(1-diag(d))+h2(1)*diag(d);
Test = diag(Test)