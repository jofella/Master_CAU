...
% between gamma and h. 
% (i) Candidate draws for h are taken from a Gamma density with mean
% mu = N/[y-f(X,gammahat)]'[y-f(X,gammahat)] and d.o.f. nu=N.
% -------------------------------------------------------------------------
...
for s = 2:(S0+S1)
    ...
    h_star = gamrnd(N/2,2/uu_hat);          % Proposal draw for h

    % Ratios of p/q for (g_star,h_star) and (gamma(s-1),h(s-1)):
    u_star = y - f(X,g_star);
    u_last = y - f(X,gamma(:,s-1));        
    pqstar_ratio = exp(-0.5*h_star*(u_star'*u_star-uu_hat) ...
        + 0.5* z_star'*z_star);
    pqlast_ratio = exp(-0.5*h(s-1)*(u_last'*u_last-uu_hat) ...
        + 0.5*(gamma(:,s-1)-gammahat)'*iSigma*(gamma(:,s-1)-gammahat));
